...
class Place_racks_and_objects():
    ...
    def place_racks_and_objects(self,rack_location,subscript,count):
        ...
        # Place the racks
        bpy.ops.wm.collada_import(filepath=self.rack_loc)
        bpy.data.objects["Shelf"]["category_id"] = 0

        # Place the Boxes
        boxes = ["BoxA", "BoxF", "BoxH", "BoxI", "BoxG"]
        #boxes = ["BoxY"]
        prev_model=None
        for rows in range(1):
        ...
        #            model = model+(append_zero(box_count[model_temp]))
                    ...
                    rot_mat = Matrix.Rotation(radians(random.uniform(0,20)), 4, 'Z')
                    ...
                    bpy.data.objects[model].location.y = cols + rack_location[1] + random.uniform(0,0.25)
                    bpy.data.objects[model].location.z = height_boxes[model_temp] #rows*1.29 + 0.35
...
def get_dimensions(obj):
    ...
    print(lst)
    #dimensions = [str(i) for i in lst]
    return dimensions

no_of_warehouses = 10 # The number of warehouse you want
...
rack_loc = './synthetic_warehouse/objects/primitives/blenderproc_objects/Rack/only_shelf/model.dae'

offset_x = -0
z_positions = []
y_positions = [-1.8, -0.3, 1.5]

try:
    os.mkdir("/scratch/warehouse/training_5000/")
except FileExistsError:
    print("Parent directory already exists")


path_img = "/scratch/warehouse/training_5000/image_2/"
path_depth = "/scratch/warehouse/training_5000/depth/"
path_calib = "/scratch/warehouse/training_5000/calib/"
path_label = "/scratch/warehouse/training_5000/label_2/"
try:
    os.mkdir(path_img)
except FileExistsError:
...
for wno in range(no_of_warehouses):
    ...
    for sno in range(no_of_scenes):
        count+=1
        camera_pos = [random.uniform(-12,-10),random.uniform(-0.5,0.5),random.uniform(0,1)]
        ...
        with open(path_label +str(count).zfill(6)+".txt", 'w') as f:
            for obj in sel_objs:
                ...
                #rotation_y = 0
                #print("The rotation %f"%(rotation_y))
                #print(location)
                #print(bbox)
...
for ii in range(no_of_warehouses*no_of_scenes):
        path = "/scratch/warehouse/training_5000/velodyne/"
        try:
            os.mkdir(path)
        except FileExistsError:
            print("Path already exists")
        
        depth = np.load("/scratch/warehouse/training_5000/depth/"+str(ii).zfill(6)+".npy")
        h,w = depth.shape
