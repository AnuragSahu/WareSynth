...
class Place_racks_and_objects():
    ...
    def place_racks_and_objects(self,rack_location,subscript,count):
        ...
        bpy.data.objects["Shelf"].location.z = -1.64

        # Place the Boxes
        boxes = ["BoxA","BoxB", "BoxC","BoxD", "BoxF", "BoxG", "BoxH", "BoxI"]
        prev_model=None
        for rows in range(1):
        ...
        #            model = model+(append_zero(box_count[model_temp]))
                    ...
                    rot_mat = Matrix.Rotation(radians(random.randint(0, 0)), 4, 'Z')
                    ...
                    bpy.data.objects[model].location.z = 0 #rows*1.29 + 0.35
                    prev_model = model
...
all_box_loc = '../synthetic_warehouse/objects/primitives/boxes_avinash/'
rack_loc = '../synthetic_warehouse/objects/primitives/Racks/Only_shelf.dae'
